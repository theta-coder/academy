      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div><h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Payment Account</h1><p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Update payment account details</p></div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <input id="account_title" v-model="form.account_title" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.account_title }" required />
              ...
              <div>
                ...
                <input id="account_number" v-model="form.account_number" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.account_number }" required />
              ...
              <div>
                ...
                <input id="bank_name" v-model="form.bank_name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              <div>
                <label for="branch_name" class="block text-sm font-medium text-gray-700 mb-2">Branch Name</label>
                <input id="branch_name" v-model="form.branch_name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              <div>
                <label for="iban" class="block text-sm font-medium text-gray-700 mb-2">IBAN</label>
                <input id="iban" v-model="form.iban" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              <div class="md:col-span-2">
                <label for="instructions" class="block text-sm font-medium text-gray-700 mb-2">Payment Instructions</label>
                <textarea id="instructions" v-model="form.instructions" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"></textarea>
              </div>
              <div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm"><span v-if="!form.processing">Update Account</span><span v-else>Updating...</span></Button>
            </div>
          </form>
        </div>
      </div>
...
const props = defineProps({ academyPaymentAccount: { type: Object, required: true } })
const form = useForm({
  account_title: props.academyPaymentAccount.account_title, payment_method: props.academyPaymentAccount.payment_method,
  account_number: props.academyPaymentAccount.account_number, bank_name: props.academyPaymentAccount.bank_name,
  branch_name: props.academyPaymentAccount.branch_name, iban: props.academyPaymentAccount.iban,
  instructions: props.academyPaymentAccount.instructions, is_active: props.academyPaymentAccount.is_active,
})
const submit = () => { form.put(route('academy-payment-accounts.update', props.academyPaymentAccount.id), { preserveScroll: true }) }
</script>
