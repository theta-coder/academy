      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Concession Types</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage fee concession categories and discount rules</p>
            </div>
            <Button @click="$inertia.visit(route('fee-concession-types.create'))" variant="primary" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <div>
              <Input v-model="filters.search" placeholder="Search concession types..." @input="searchDebounced" class="w-full text-sm" />
            </div>
            <div>
            ...
            <table id="data-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Concession Name</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Discount Type</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Default Value</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Applies To</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <div v-else-if="mobileItems.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            ...
            <p class="mt-2 text-sm font-medium text-gray-500">No concession types found</p>
          </div>
            ...
            <div class="p-4">
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ item.concession_name }}</h3>
                  </div>
                </div>
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  <span class="text-gray-500 w-28">Discount Type:</span>
                  <span class="px-2 py-0.5 text-xs font-medium rounded-full" :class="item.discount_type === 'percentage' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'">{{ item.discount_type === 'percentage' ? 'Percentage' : 'Fixed' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <span class="text-gray-500 w-28">Default Value:</span>
                  <span class="text-gray-900 font-medium">{{ item.discount_type === 'percentage' ? item.default_discount_value + '%' : 'Rs. ' + item.default_discount_value }}</span>
                </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button @click="$inertia.visit(route('fee-concession-types.edit', item.id))" class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors flex items-center justify-center gap-1">
                ...
                <button @click="() => { itemToDelete = item.id; showDeleteModal = true; }" class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors flex items-center justify-center gap-1">
        ...
        <div v-if="mobileItems.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            <button @click="prevPage" :disabled="mobileCurrentPage === 1 || mobileLoading" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed bg-white hover:bg-gray-50 transition-colors flex items-center gap-1">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total</div>
            </div>
            <button @click="nextPage" :disabled="mobileCurrentPage === mobileTotalPages || mobileLoading" class="px-4 py-2 text-sm font-medium border border-gray-300 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed bg-white hover:bg-gray-50 transition-colors flex items-center gap-1">
        ...
        <template #title>
          <div class="flex items-center">
            <div class="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-red-100 flex items-center justify-center mr-3 sm:mr-4">
              <svg class="w-5 h-5 sm:w-6 sm:h-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
            </div>
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Concession Type</span>
          </div>
        </template>
        <p class="text-xs sm:text-sm text-gray-600 mt-2">Are you sure you want to delete this concession type? This action cannot be undone.</p>
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            <Button variant="secondary" @click="showDeleteModal = false" class="w-full sm:w-auto px-4 sm:px-6 shadow-sm hover:shadow-md transition-all text-sm">Cancel</Button>
            <Button variant="danger" @click="confirmDelete" :loading="deleting" class="w-full sm:w-auto px-4 sm:px-6 shadow-md hover:shadow-lg transition-all text-sm">
              <span v-if="!deleting">Delete</span><span v-else>Deleting...</span>
            </Button>
          </div>
        </template>
      </Modal>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('fee-concession-types.index'), { params, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
...
onMounted(() => {
  ...
  table = $('#data-table').DataTable({
    ...
    ajax: {
      url: route('fee-concession-types.index'),
      data: function(d) {
    ...
    columns: [
      ...
      { data: 'concession_name', name: 'concession_name' },
      { data: 'discount_type', name: 'discount_type' },
      { data: 'default_discount_value', name: 'default_discount_value' },
      { data: 'applies_to', name: 'applies_to' },
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><p class="mt-2 text-sm font-medium">No concession types found</p></div>',
    ...
    drawCallback: function () {
      $('#data-table_info').appendTo('#table-info')
      $('#data-table_paginate').appendTo('#table-pagination')
    }
  })
})
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('fee-concession-types.destroy', itemToDelete.value), {
    onSuccess: () => { showDeleteModal.value = false; deleting.value = false; loadData() },
    onError: () => { deleting.value = false }
  })
}
window.editConcessionType = (item) => { router.visit(route('fee-concession-types.edit', item.id)) }
window.deleteConcessionType = (id) => { itemToDelete.value = id; showDeleteModal.value = true }
