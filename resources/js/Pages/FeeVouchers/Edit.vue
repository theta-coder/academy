      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Fee Voucher</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Update voucher details</p>
            </div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <select id="student_enrollment_id" v-model="form.student_enrollment_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
              ...
              <div>
                ...
                <input id="voucher_no" v-model="form.voucher_no" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              <div>
              ...
              <div>
                ...
                <select id="status" v-model="form.status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                  <option value="pending">Pending</option><option value="partial">Partial</option><option value="paid">Paid</option><option value="waived">Waived</option><option value="cancelled">Cancelled</option>
                </select>
              </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Update Voucher</span><span v-else>Updating...</span>
              </Button>
            </div>
          </form>
        </div>
...
const props = defineProps({ voucher: { type: Object, required: true }, enrollments: Array, feeTypes: Array, academicYears: Array })
const form = useForm({
  student_enrollment_id: props.voucher.student_enrollment_id, fee_type_id: props.voucher.fee_type_id,
  academic_year_id: props.voucher.academic_year_id, voucher_no: props.voucher.voucher_no ?? '',
  month: props.voucher.month, year: props.voucher.year, generated_for: props.voucher.generated_for ?? '',
  original_amount: props.voucher.original_amount, discount_amount: props.voucher.discount_amount ?? 0,
  fine_amount: props.voucher.fine_amount ?? 0, net_amount: props.voucher.net_amount,
  paid_amount: props.voucher.paid_amount ?? 0, remaining_amount: props.voucher.remaining_amount ?? 0,
  due_date: props.voucher.due_date ?? '', status: props.voucher.status ?? 'pending', notes: props.voucher.notes ?? '',
})
const submit = () => { form.put(route('fee-vouchers.update', props.voucher.id), { preserveScroll: true }) }
</script>
