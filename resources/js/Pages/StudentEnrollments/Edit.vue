      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Enrollment</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Update student enrollment details</p>
            </div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <input id="roll_number" v-model="form.roll_number" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              ...
              <!-- Leaving Date -->
              <div>
                <label for="leaving_date" class="block text-sm font-medium text-gray-700 mb-2">Leaving Date</label>
                <input id="leaving_date" v-model="form.leaving_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              ...
              <div>
                ...
                <input id="sibling_order" v-model="form.sibling_order" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
              </div>
              ...
              <div>
                ...
                <select id="status" v-model="form.status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                  <option value="active">Active</option><option value="left">Left</option><option value="graduated">Graduated</option><option value="suspended">Suspended</option>
                </select>
              </div>

              <!-- Leaving Reason -->
              <div class="col-span-2">
                <label for="leaving_reason" class="block text-sm font-medium text-gray-700 mb-2">Leaving Reason</label>
                <textarea id="leaving_reason" v-model="form.leaving_reason" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Reason for leaving..."></textarea>
              </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Update Enrollment</span>
...
const classOptions = ref(props.initialClasses)
const sectionOptions = ref(props.initialSections)
const loadingClasses = ref(false)
const loadingSections = ref(false)
const selectedBranchClassId = ref(props.enrollment.branch_class_id ?? '')

const form = useForm({
  student_id: props.enrollment.student_id,
  academic_year_id: props.enrollment.academic_year_id,
  branch_id: props.enrollment.branch_id,
  class_section_id: props.enrollment.class_section_id,
  roll_number: props.enrollment.roll_number ?? '',
  admission_date: props.enrollment.admission_date ?? '',
  leaving_date: props.enrollment.leaving_date ?? '',
  enrollment_type: props.enrollment.enrollment_type ?? 'new',
  sibling_order: props.enrollment.sibling_order ?? '',
  status: props.enrollment.status ?? 'active',
  leaving_reason: props.enrollment.leaving_reason ?? '',
  remarks: props.enrollment.remarks ?? '',
})
...
const submit = () => {
  form.put(route('student-enrollments.update', props.enrollment.id), {
    preserveScroll: true,
  })
}
</script>
