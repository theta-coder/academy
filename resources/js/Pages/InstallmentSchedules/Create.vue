      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div><h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Installment Kist</h1><p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Add a new kist to an installment assignment</p></div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <input id="kist_number" v-model="form.kist_number" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.kist_number }" placeholder="e.g., 1" required />
              ...
              <div>
                ...
                <input id="kist_amount" v-model="form.kist_amount" type="number" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.kist_amount }" required />
              ...
              <div>
                ...
                <input id="due_date" v-model="form.due_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.due_date }" required />
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm"><span v-if="!form.processing">Create Kist</span><span v-else>Creating...</span></Button>
            </div>
          </form>
        </div>
      </div>
...
const form = useForm({ assignment_id: '', kist_number: '', kist_amount: '', due_date: '', status: 'pending', notes: '' })
const submit = () => { form.post(route('installment-schedules.store'), { preserveScroll: true }) }
</script>
