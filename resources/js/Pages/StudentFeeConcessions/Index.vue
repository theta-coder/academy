      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Student Fee Concessions</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage fee concessions assigned to students</p>
            </div>
            <Button
              @click="$inertia.visit(route('student-fee-concessions.create'))"
              variant="primary"
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="concessions-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Concession Type</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Discount</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Dates</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileConcessions.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z"/>
            </svg>
            <p class="mt-2 text-sm font-medium text-gray-500">No concessions found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Concession Cards -->
          <div v-else v-for="(concession, index) in mobileConcessions" :key="concession.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
                ...
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">
                      {{ concession.student_enrollment?.student?.student_name || 'N/A' }}
                    </h3>
                  </div>
                </div>
                <span :class="getStatusClass(concession.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">Concession: {{ concession.concession_type?.concession_name || '—' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
                  </svg>
                  <span class="text-gray-600">Fee Type: {{ concession.fee_type?.fee_name || 'All' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
                  </svg>
                  <span class="text-gray-600">Discount: {{ formatDiscount(concession.discount_value, concession.discount_type) }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <span class="text-gray-600">
                    {{ formatDate(concession.start_date) }} → {{ concession.end_date ? formatDate(concession.end_date) : 'Ongoing' }}
                  </span>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                ...
                <button
                  @click="() => { concessionToDelete = concession.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileConcessions.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total concessions</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Fee Concession</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Concession</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('student-fee-concessions.index'), {
      params,
      headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
    })

    if (response.data?.data) {
      mobileConcessions.value = response.data.data
      ...
      mobileConcessions.value = response.data
...
onMounted(() => {
  ...
  table = $('#concessions-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
      url: route('student-fee-concessions.index'),
      data: function (d) {
    ...
    columns: [
      ...
      { data: 'concession_type', name: 'concession_type', orderable: false },
    ...
    drawCallback: function () {
      $('#table-info').empty().append($('#concessions-table_info'))
      $('#table-pagination').empty().append($('#concessions-table_paginate'))
    }
  })
})

// Global handlers for DataTables action buttons
window.editConcession = (concession) => {
  router.visit(route('student-fee-concessions.edit', concession.id))
}
window.deleteConcession = (id) => {
  concessionToDelete.value = id
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('student-fee-concessions.destroy', concessionToDelete.value), {
    onSuccess: () => {
...
:deep(#concessions-table_info),
:deep(#concessions-table_paginate) {
  display: none;
}
...
:deep(#concessions-table tbody td) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
}
...
@media (max-width: 1024px) {
  :deep(#concessions-table) { font-size: 0.813rem; }
  :deep(#concessions-table th),
  :deep(#concessions-table td) { padding: 0.5rem; }
}
</style>