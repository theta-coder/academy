      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div><h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Advance Adjustment</h1><p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Update adjustment details</p></div>
          ...
          <form @submit.prevent="submit">
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Update Adjustment</span><span v-else>Updating...</span>
              </Button>
            </div>
          </form>
        </div>
...
const props = defineProps({ adjustment: { type: Object, required: true } })
const form = useForm({ student_enrollment_id: props.adjustment.student_enrollment_id, from_payment_id: props.adjustment.from_payment_id, to_voucher_id: props.adjustment.to_voucher_id, adjusted_amount: props.adjustment.adjusted_amount, adjusted_at: props.adjustment.adjusted_at ?? '', notes: props.adjustment.notes ?? '' })
