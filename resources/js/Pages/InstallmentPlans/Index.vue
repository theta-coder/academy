      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Installment Plans</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage fee installment payment plans</p>
            </div>
            <Button @click="$inertia.visit(route('installment-plans.create'))" variant="primary" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
              <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Add Plan
            </Button>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <div><Input v-model="filters.search" placeholder="Search plans..." @input="searchDebounced" class="w-full text-sm" /></div>
            ...
            <table id="data-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Plan Name</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Total Installments</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <div v-else-if="mobileItems.length === 0" class="text-center py-12 bg-white rounded-lg shadow"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg><p class="mt-2 text-sm font-medium text-gray-500">No installment plans found</p></div>
          <div v-else v-for="(item, index) in mobileItems" :key="item.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1"><div class="flex items-center gap-2"><span class="text-xs font-semibold text-gray-500">#{{ mobileOffset + index + 1 }}</span><h3 class="text-base font-semibold text-gray-900">{{ item.plan_name }}</h3></div></div>
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-28">Installments:</span><span class="font-semibold text-indigo-600">{{ item.total_installments }}</span></div>
                <div class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-28">Fee Type:</span><span class="text-gray-900">{{ item.fee_type?.fee_name || 'All Fee Types' }}</span></div>
              </div>
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button @click="$inertia.visit(route('installment-plans.edit', item.id))" class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors flex items-center justify-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Edit</button>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('installment-plans.index'), { params, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
...
onMounted(() => {
  ...
  table = $('#data-table').DataTable({
    ...
    ajax: { url: route('installment-plans.index'), data: function(d) { d.search.value = filters.search || tableSearch.value; if (filters.is_active !== '') d.is_active = filters.is_active } },
    columns: [
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><p class="mt-2 text-sm font-medium">No plans found</p></div>',
...
const confirmDelete = () => { deleting.value = true; router.delete(route('installment-plans.destroy', itemToDelete.value), { onSuccess: () => { showDeleteModal.value = false; deleting.value = false; loadData() }, onError: () => { deleting.value = false } }) }
window.editPlan = (item) => { router.visit(route('installment-plans.edit', item.id)) }
window.deletePlan = (id) => { itemToDelete.value = id; showDeleteModal.value = true }
