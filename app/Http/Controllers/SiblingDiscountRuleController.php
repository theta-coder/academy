...
class SiblingDiscountRuleController extends Controller
{
    ...
    public function edit(SiblingDiscountRule $siblingDiscountRule)
    {
        return Inertia::render('SiblingDiscountRules/Edit', [
            'rule' => [
                'id' => $siblingDiscountRule->id, 'child_number' => $siblingDiscountRule->child_number,
                'discount_type' => $siblingDiscountRule->discount_type, 'discount_value' => $siblingDiscountRule->discount_value,
                'applies_to_fee_type_id' => $siblingDiscountRule->applies_to_fee_type_id,
                'description' => $siblingDiscountRule->description, 'is_active' => $siblingDiscountRule->is_active,
            ],
            'feeTypes' => FeeType::select('id', 'fee_name')->orderBy('fee_name')->get(),
        ]);
    }
    ...
    private function getDataTablesRules(Request $request)
    {
        ...
        $data = $rules->map(function ($rule, $index) use ($start) {
            ...
            return [
                ...
                'action' => '<div class="flex items-center justify-center gap-2"><button onclick=\'editRule(' . json_encode(['id' => $rule->id]) . ')\' class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Edit</button><button onclick="deleteRule(' . $rule->id . ')" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>Delete</button></div>'
            ];
        });
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'child_number' => 'required|integer|min:1', 'discount_type' => 'required|string|in:percentage,fixed',
            'discount_value' => 'required|numeric|min:0', 'applies_to_fee_type_id' => 'nullable|exists:fee_types,id',
