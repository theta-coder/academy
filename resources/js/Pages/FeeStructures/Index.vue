      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Fee Structures Management</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage all fee structures</p>
            </div>
            <Button
              @click="$inertia.visit(route('fee-structures.create'))"
              variant="primary"
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="fee-structures-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Class</th>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Due Day</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <div v-else-if="mobileItems.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            ...
            <p class="mt-2 text-sm font-medium text-gray-500">No fee structures found</p>
            ...
            <div class="p-4">
              ...
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ item.fee_type?.fee_name ?? '-' }}</h3>
                  </div>
                </div>
                <span :class="getStatusClass(item.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">Year: {{ item.academic_year?.year_name ?? '-' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                  </svg>
                  <span class="text-gray-600">Branch: {{ item.branch?.branch_name ?? '-' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                  </svg>
                  <span class="text-gray-600">Amount: Rs. {{ Number(item.amount).toLocaleString() }}</span>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button
                  @click="$inertia.visit(route('fee-structures.edit', item.id))"
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Fee Structure</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Structure</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('fee-structures.index'), {
      params,
...
onMounted(() => {
  ...
  table = $('#fee-structures-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
    ...
    columns: [
      ...
      { data: 'academic_year', name: 'academic_year_id' },
      { data: 'branch_name', name: 'branch_id' },
      { data: 'class_name', name: 'class_id' },
      { data: 'fee_type', name: 'fee_type_id' },
      { data: 'amount', name: 'amount' },
      { data: 'due_day', name: 'due_day', orderable: false, searchable: false },
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><p class="mt-2 text-sm font-medium">No fee structures found</p></div>',
    ...
    drawCallback: function () {
      $('#table-info').empty().append($('#fee-structures-table_info'))
      $('#table-pagination').empty().append($('#fee-structures-table_paginate'))
    }
  })
})

// Edit
window.editStructure = (struct) => {
  router.visit(route('fee-structures.edit', struct.id))
}

// Delete
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('fee-structures.destroy', itemToDelete.value), {
    onSuccess: () => {
...
:deep(#fee-structures-table_info),
:deep(#fee-structures-table_paginate) {
  display: none;
}
...
:deep(#fee-structures-table tbody td) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
}
...
@media (max-width: 1024px) {
  :deep(#fee-structures-table) { font-size: 0.813rem; }
  :deep(#fee-structures-table th),
  :deep(#fee-structures-table td) { padding: 0.5rem; }
}
</style>
