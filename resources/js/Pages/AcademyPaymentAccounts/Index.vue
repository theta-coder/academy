      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Payment Accounts</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage academy bank accounts and payment methods</p>
            </div>
            <Button @click="$inertia.visit(route('academy-payment-accounts.create'))" variant="primary" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
              <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>Add Account
            </Button>
          </div>
        </div>

        <div v-if="showSuccessMessage && $page.props.flash.success" class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded-lg relative"><div class="flex items-center justify-between"><div class="flex items-center"><svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>{{ $page.props.flash.success }}</div><button @click="showSuccessMessage = false" class="text-green-700 hover:text-green-900"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></button></div></div>
        <div v-if="showErrorMessage && $page.props.flash.error" class="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative"><div class="flex items-center justify-between"><div class="flex items-center"><svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"/></svg>{{ $page.props.flash.error }}</div><button @click="showErrorMessage = false" class="text-red-700 hover:text-red-900"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></button></div></div>

        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            <div><Input v-model="filters.search" placeholder="Search accounts..." @input="searchDebounced" class="w-full text-sm" /></div>
            ...
            <table id="data-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Account Title</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Payment Method</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Account Number</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Bank Name</th>
          ...
          <div v-else v-for="(item, index) in mobileItems" :key="item.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1"><div class="flex items-center gap-2"><span class="text-xs font-semibold text-gray-500">#{{ mobileOffset + index + 1 }}</span><h3 class="text-base font-semibold text-gray-900">{{ item.account_title }}</h3></div></div>
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-32">Method:</span><span class="px-2 py-0.5 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800">{{ item.payment_method }}</span></div>
                <div class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-32">Account No:</span><span class="text-gray-900 font-mono">{{ item.account_number }}</span></div>
                <div v-if="item.bank_name" class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-32">Bank:</span><span class="text-gray-900">{{ item.bank_name }}</span></div>
                <div v-if="item.iban" class="flex items-center text-xs sm:text-sm"><span class="text-gray-500 w-32">IBAN:</span><span class="text-gray-900 font-mono text-xs">{{ item.iban }}</span></div>
              </div>
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button @click="$inertia.visit(route('academy-payment-accounts.edit', item.id))" class="flex-1 px-3 py-2 text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors flex items-center justify-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>Edit</button>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    if (filters.search) params.search = filters.search; if (tableSearch.value) params.search = tableSearch.value
    if (filters.is_active !== '') params.is_active = filters.is_active
    const response = await axios.get(route('academy-payment-accounts.index'), { params, headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
...
onMounted(() => {
  ...
  table = $('#data-table').DataTable({
    ...
    ajax: { url: route('academy-payment-accounts.index'), data: function(d) { d.search.value = filters.search || tableSearch.value; if (filters.is_active !== '') d.is_active = filters.is_active } },
    columns: [
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><p class="mt-2 text-sm font-medium">No accounts found</p></div>',
...
const confirmDelete = () => { deleting.value = true; router.delete(route('academy-payment-accounts.destroy', itemToDelete.value), { onSuccess: () => { showDeleteModal.value = false; deleting.value = false; loadData() }, onError: () => { deleting.value = false } }) }
window.editAccount = (item) => { router.visit(route('academy-payment-accounts.edit', item.id)) }
window.deleteAccount = (id) => { itemToDelete.value = id; showDeleteModal.value = true }
