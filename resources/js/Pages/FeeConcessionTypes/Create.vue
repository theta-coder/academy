      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Concession Type</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Add a new fee concession category</p>
            </div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <input id="concession_name" v-model="form.concession_name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.concession_name }" placeholder="e.g., Staff Child Discount" required />
              ...
              <div>
                ...
                <input id="default_discount_value" v-model="form.default_discount_value" type="number" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.default_discount_value }" placeholder="e.g., 10" required />
              ...
              <div class="md:col-span-2">
                ...
                <textarea id="description" v-model="form.description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Additional details..."></textarea>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Create Concession Type</span><span v-else>Creating...</span>
              </Button>
            </div>
          </form>
        </div>
...
const form = useForm({
  concession_name: '', discount_type: '', default_discount_value: '', applies_to: '', description: '', is_active: true,
})

const submit = () => {
  form.post(route('fee-concession-types.store'), { preserveScroll: true })
}
</script>
