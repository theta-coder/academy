      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div><h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create New Parent</h1><p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Add a new parent/guardian to the system</p></div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md overflow-hidden">
          <form @submit.prevent="submit">
            ...
            <div class="p-4 sm:p-6 lg:p-8 border-b border-gray-100">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div><label for="mother_name" class="block text-sm font-medium text-gray-700 mb-2">Mother's Name</label><input id="mother_name" v-model="form.mother_name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Enter mother's name" /></div>
                <div><label for="mother_cnic" class="block text-sm font-medium text-gray-700 mb-2">Mother's CNIC</label><input id="mother_cnic" v-model="form.mother_cnic" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="12345-1234567-1" /></div>
                <div><label for="mother_phone" class="block text-sm font-medium text-gray-700 mb-2">Mother's Phone</label><input id="mother_phone" v-model="form.mother_phone" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="0300-1234567" /></div>
              </div>
            </div>
            <!-- Address & Emergency -->
            <div class="p-4 sm:p-6 lg:p-8 bg-gray-50 border-b border-gray-100">
            ...
            <div class="px-4 sm:px-6 lg:px-8 py-6 bg-gray-50 border-t border-gray-200">
              <div class="flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
                ...
                <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm"><span v-if="!form.processing">Create Parent</span><span v-else>Creating...</span></Button>
              </div>
            </div>
          </form>
        </div>
...
const form = useForm({ father_name: '', father_cnic: '', father_phone: '', father_occupation: '', mother_name: '', mother_cnic: '', mother_phone: '', address: '', city: '', emergency_contact_name: '', emergency_contact_phone: '', is_active: true })
const submit = () => { form.post(route('parents.store'), { preserveScroll: true }) }
</script>