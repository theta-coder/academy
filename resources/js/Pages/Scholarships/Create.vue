      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Scholarship</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Add a new scholarship program</p>
            </div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                <input id="scholarship_name" v-model="form.scholarship_name" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.scholarship_name }" placeholder="e.g., Merit Scholarship" required />
              ...
              <div>
                ...
                <p v-if="form.errors.applicable_fee_type_id" class="mt-1 text-sm text-red-600">{{ form.errors.applicable_fee_type_id }}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <input id="criteria" v-model="form.criteria" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="e.g., Top 3 position holders" />
              </div>
              <div>
              ...
              <div>
                ...
                <input id="max_recipients" v-model="form.max_recipients" type="number" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="e.g., 10" />
              </div>
              <div>
              ...
              <div class="md:col-span-2">
                ...
                <textarea id="description" v-model="form.description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Scholarship details..."></textarea>
              </div>
            </div>
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm"><span v-if="!form.processing">Create Scholarship</span><span v-else>Creating...</span></Button>
            </div>
          </form>
        </div>
      </div>
...
const form = useForm({
  scholarship_name: '', criteria: '', discount_type: '', discount_value: '', applies_to: '',
  applicable_fee_type_id: '', max_recipients: '', is_renewable: false, description: '', is_active: true,
})

const submit = () => { form.post(route('scholarships.store'), { preserveScroll: true }) }
</script>
