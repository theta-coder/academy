      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Student Scholarships</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage scholarships awarded to students</p>
            </div>
            <Button
              @click="$inertia.visit(route('student-scholarships.create'))"
              variant="primary"
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="scholarships-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Scholarship</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Academic Year</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Awarded On</th>
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Valid Period</th>
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <!-- Empty State -->
          <div v-else-if="mobileScholarships.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
          ...
          <!-- Scholarship Cards -->
          <div v-else v-for="(scholarship, index) in mobileScholarships" :key="scholarship.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
                ...
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">
                      {{ scholarship.student_enrollment?.student?.student_name || 'N/A' }}
                    </h3>
                  </div>
                </div>
                <span :class="getStatusClass(scholarship.status)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2 capitalize">
                  {{ scholarship.status }}
                </span>
              </div>
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">Scholarship: {{ scholarship.scholarship?.scholarship_name || '—' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                  </svg>
                  <span class="text-gray-600">Year: {{ scholarship.academic_year?.year_name || '—' }}</span>
                </div>
                <div class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                  </svg>
                  <span class="text-gray-600">Awarded: {{ formatDate(scholarship.awarded_on) }}</span>
                </div>
                <div v-if="scholarship.marks_percentage" class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                  </svg>
                  <span class="text-gray-600">Marks: {{ scholarship.marks_percentage }}%</span>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                ...
                <button
                  @click="() => { scholarshipToDelete = scholarship.id; showDeleteModal = true; }"
        ...
        <!-- Mobile Pagination -->
        <div v-if="mobileScholarships.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total scholarships</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Scholarship</span>
          </div>
        </template>

        <p class="text-xs sm:text-sm text-gray-600 mt-2">
          Are you sure you want to remove this scholarship? This action cannot be undone.
        </p>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Scholarship</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('student-scholarships.index'), {
      params,
      headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
    })

    if (response.data?.data) {
      mobileScholarships.value = response.data.data
      ...
      mobileScholarships.value = response.data
...
onMounted(() => {
  ...
  table = $('#scholarships-table').DataTable({
    processing: true,
    serverSide: true,
    ajax: {
    ...
    columns: [
      ...
      { data: 'scholarship_name', name: 'scholarship_name', orderable: false },
      { data: 'year_name',        name: 'year_name',        orderable: false },
      { data: 'awarded_on',       name: 'awarded_on' },
      { data: 'valid_period',     name: 'valid_period',     orderable: false },
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 7l9-5-9-5-9 5 9 5z"/></svg><p class="mt-2 text-sm font-medium">No scholarships found</p></div>',
    ...
    drawCallback: function () {
      $('#table-info').empty().append($('#scholarships-table_info'))
      $('#table-pagination').empty().append($('#scholarships-table_paginate'))
    }
  })
})
...
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('student-scholarships.destroy', scholarshipToDelete.value), {
    onSuccess: () => {
...
:deep(#scholarships-table_info),
:deep(#scholarships-table_paginate) {
  display: none;
}
...
:deep(#scholarships-table tbody td) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
}
...
@media (max-width: 1024px) {
  :deep(#scholarships-table) { font-size: 0.813rem; }
  :deep(#scholarships-table th),
  :deep(#scholarships-table td) { padding: 0.5rem; }
}
</style>
