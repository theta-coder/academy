      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Award Scholarship</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Assign a scholarship to an enrolled student</p>
            </div>
        ...
        <div class="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6">
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <!-- Awarded On -->
              <div>
                <label for="awarded_on" class="block text-sm font-medium text-gray-700 mb-2">Awarded On <span class="text-red-500">*</span></label>
                <input id="awarded_on" v-model="form.awarded_on" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.awarded_on }" />
                <p v-if="form.errors.awarded_on" class="mt-1 text-sm text-red-600">{{ form.errors.awarded_on }}</p>
              </div>

              <!-- Valid From -->
              <div>
                <label for="valid_from" class="block text-sm font-medium text-gray-700 mb-2">Valid From <span class="text-red-500">*</span></label>
                <input id="valid_from" v-model="form.valid_from" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" :class="{ 'border-red-500': form.errors.valid_from }" />
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Award Scholarship</span>
...
const submit = () => {
  form.post(route('student-scholarships.store'), { preserveScroll: true })
}
</script>
