      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div><h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Collection Summary</h1><p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Record a fee collection summary</p></div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <select v-model="form.month" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                  <option value="">Select</option><option v-for="m in 12" :key="m" :value="m">{{ m }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Year <span class="text-red-500">*</span></label>
                <input v-model="form.year" type="number" min="2020" max="2040" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required />
              </div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Create Summary</span><span v-else>Creating...</span>
              </Button>
            </div>
          </form>
        </div>
...
const props = defineProps({ branches: Array, academicYears: Array })
const form = useForm({ branch_id: '', academic_year_id: '', month: '', year: new Date().getFullYear(), total_students: 0, total_billed: 0, total_discount: 0, total_fine: 0, total_collected: 0, total_pending: 0 })
const submit = () => { form.post(route('fee-collection-summaries.store'), { preserveScroll: true }) }
</script>
