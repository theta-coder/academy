      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="md:hidden space-y-3 sm:space-y-4">
          ...
          <div v-else-if="mobileClasses.length === 0" class="text-center py-12 bg-white rounded-lg shadow">
            ...
            <p class="mt-2 text-sm font-medium text-gray-500">No classes found</p>
            <p class="mt-1 text-xs text-gray-400">Try adjusting your filters</p>
          </div>

          <!-- Class Cards -->
          <div v-else v-for="(classItem, index) in mobileClasses" :key="classItem.id" class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
            <div class="p-4">
              <!-- Header -->
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ classItem.class_name }}</h3>
                  </div>
                </div>
                <span :class="getStatusClass(classItem.is_active)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2">
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                ...
                <div class="flex items-center justify-between text-xs sm:text-sm">
                  ...
                  <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800">
                    {{ classItem.branch_classes_count || 0 }} branches
                  </span>
                </div>
              </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Class</span>
...
onMounted(() => {
  ...
  table = $('#classes-table').DataTable({
    ...
    columns: [
      ...
      { data: 'branch_classes_count', name: 'branch_classes_count', orderable: false, searchable: false },
