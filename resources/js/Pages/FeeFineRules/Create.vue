      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Create Fine Rule</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Set up a new late payment fine rule</p>
            </div>
          ...
          <form @submit.prevent="submit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <p v-if="form.errors.days_after_due" class="mt-1 text-sm text-red-600">{{ form.errors.days_after_due }}</p>
              </div>
              <div>
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-end gap-3 sm:gap-4">
              ...
              <Button type="submit" variant="primary" :loading="form.processing" class="w-full sm:w-auto shadow-lg hover:shadow-xl transition-all text-sm">
                <span v-if="!form.processing">Create Fine Rule</span><span v-else>Creating...</span>
              </Button>
            </div>
          </form>
        </div>
...
const props = defineProps({ branches: Array, feeTypes: Array })
const form = useForm({ branch_id: '', fee_type_id: '', applies_to_all_fee_types: false, days_after_due: '', fine_type: 'fixed', fine_value: '', max_fine: '', description: '', is_active: true })
const submit = () => { form.post(route('fee-fine-rules.store'), { preserveScroll: true }) }
</script>
