      ...
      <div class="flex-1 px-3 sm:px-4 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="mb-4 sm:mb-6 lg:mb-8">
          <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Student Enrollments</h1>
              <p class="mt-1 sm:mt-2 text-xs sm:text-sm text-gray-600">Manage student enrollment records</p>
            </div>
            <Button
              @click="$inertia.visit(route('student-enrollments.create'))"
              variant="primary"
        ...
        <div class="hidden md:block bg-white rounded-lg sm:rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table id="enrollments-table" class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gradient-to-r from-indigo-50 to-blue-50">
                <tr>
                  ...
                  <th class="px-3 sm:px-6 py-3 sm:py-4 text-xs font-semibold text-gray-700 uppercase tracking-wider text-center">Class Section</th>
            ...
            <div class="p-4">
              ...
              <div class="flex items-start justify-between mb-3">
                <div class="flex-1">
                  <div class="flex items-center gap-2">
                    ...
                    <h3 class="text-base font-semibold text-gray-900">{{ item.student?.student_name || 'N/A' }}</h3>
                  </div>
                </div>
                <span :class="getStatusClass(item.status)" class="px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ml-2 capitalize">
                  {{ item.status }}
                </span>
              </div>
              ...
              <div class="space-y-2 border-t border-gray-100 pt-3">
                ...
                <div class="flex items-center text-xs sm:text-sm">
                  ...
                  <span class="text-gray-600">Class: {{ item.class_section?.section?.section_name || '—' }}</span>
                </div>
                <div v-if="item.roll_number" class="flex items-center text-xs sm:text-sm">
                  <svg class="w-4 h-4 text-gray-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14"/>
                  </svg>
                  <span class="text-gray-600">Roll No: {{ item.roll_number }}</span>
                </div>
              </div>
              ...
              <div class="flex gap-2 mt-4 pt-3 border-t border-gray-100">
                <button
                  @click="$inertia.visit(route('student-enrollments.edit', item.id))"
        ...
        <div v-if="mobileItems.length > 0" class="md:hidden mt-4 bg-white rounded-lg shadow p-3">
          <div class="flex items-center justify-between">
            ...
            <div class="text-center">
              ...
              <div class="text-xs text-gray-500 mt-0.5">{{ mobileTotal }} total enrollments</div>
            </div>
      ...
      <Modal :show="showDeleteModal" @close="showDeleteModal = false">
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-base sm:text-lg font-semibold text-gray-900">Delete Enrollment</span>
          </div>
        </template>
        ...
        <template #footer>
          <div class="flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
            ...
            >
              <span v-if="!deleting">Delete Enrollment</span>
...
const loadMobileData = async () => {
  ...
  try {
    ...
    const response = await axios.get(route('student-enrollments.index'), {
      params,
...
onMounted(() => {
  ...
  table = $('#enrollments-table').DataTable({
    ...
    columns: [
      ...
      { data: 'enrollment_type', name: 'enrollment_type' },
    ...
    language: {
      emptyTable: '<div class="text-center py-12 text-gray-500"><svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg><p class="mt-2 text-sm font-medium">No enrollments found</p></div>',
    ...
    drawCallback: function () {
      $('#table-info').empty().append($('#enrollments-table_info'))
      $('#table-pagination').empty().append($('#enrollments-table_paginate'))
    }
  })
})

// Edit
window.editEnrollment = (item) => {
  router.visit(route('student-enrollments.edit', item.id))
}

// Delete
const confirmDelete = () => {
  deleting.value = true
  router.delete(route('student-enrollments.destroy', itemToDelete.value), {
    onSuccess: () => {
...
:deep(#enrollments-table_info),
:deep(#enrollments-table_paginate) {
  display: none;
}
...
:deep(#enrollments-table tbody td) {
  padding: 0.5rem 0.75rem;
  font-size: 0.875rem;
}
...
@media (max-width: 1024px) {
  :deep(#enrollments-table) { font-size: 0.813rem; }
  :deep(#enrollments-table th),
  :deep(#enrollments-table td) { padding: 0.5rem; }
}
</style>